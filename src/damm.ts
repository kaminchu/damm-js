...
export function computeAlphabet(input: string){
  ...
  return alphabet.charAt(compute(get26x26(), input.split('').map((s) => alphabet.indexOf(s))));
}
...
export function computeAlphaNumeric(input: string){
  ...
  return alphaNumeric.charAt(compute(get36x36(), input.split('').map((s) => alphaNumeric.indexOf(s))));
}
...
export function computeMixedCaseAlphaNumeric(input: string){
  ...
  const mixedCaseAlphaNumeric = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
  return mixedCaseAlphaNumeric.charAt(compute(get62x62(), input.split('').map((s) => mixedCaseAlphaNumeric.indexOf(s))));
}
